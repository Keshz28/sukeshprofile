...
const serialize = (d: unknown) => JSON.stringify(d, null, 2) + "\n";
...
const blankSkillCat = (): SkillCategory => ({ title: "", accent: "blue", skills: [] });
const blankTool = (): AiTool => ({ name: "", org: "", role: "", accent: "blue", points: [] });
const blankCert = (): Certification => ({ title: "", issuer: "", year: "", skills: [] });
...
export default function Editor({
  ...
  const handleSave = async () => {
    try {
      await onSave(draft);
      setSavedJson(serialize(draft));
    } catch {
      /* parent surfaces the error via `status` */
    }
  };
    ...
    <main className="mx-auto min-h-screen max-w-[1000px] px-[clamp(16px,4vw,48px)] py-10">
      <header className="mb-6">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={onLogout}
            className="ml-auto font-mono text-[11px] uppercase tracking-[0.12em] text-white/45 transition hover:text-white"
          >
            Lock ↩
          </button>
        </div>
        <p className="mt-2 max-w-[70ch] text-sm leading-[1.6] text-white/55">
          Saving commits <code className="rounded bg-white/[0.06] px-1.5 py-0.5 font-mono text-[12px] text-blue-glow">lib/content.json</code>{" "}
          to your repo, which auto-redeploys the portfolio (~30–60s).
        </p>
      </header>
      ...
      <div className="sticky top-3 z-10 mb-6 flex flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-ink/80 p-2.5 backdrop-blur-xl">
        <button
          ...
          className="rounded-lg bg-brand-gradient px-4 py-2 font-display text-sm font-bold text-ink transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
        >
          {saving ? "Saving…" : "Save & publish →"}
        </button>
        <div className="ml-auto flex items-center gap-3">
          {status && (
            <span className="font-mono text-[11px] text-blue-glow">{status}</span>
          )}
          <button
